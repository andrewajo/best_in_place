  ...
  loadSuccessCallback : function(data) {
    ...
    if (response != null) {
	      ...
	      this.element.html(response["display_as"]);
				this.element.removeClass("no_info");
			} else {
				for (var key in response) {
					if (response.hasOwnProperty(key)) {
						var sibling = $("#" + this.element.attr("id").replace(this.attributeName, key));
						var values = $.parseJSON(response[key]);
						sibling.attr("data-original-content", values["original_value"]);
						sibling.original_content = values["original_value"];
						var sibling_html = (values.hasOwnProperty("display_as")) ? values["display_as"] : values["original_value"];
						sibling.html(sibling_html);
						sibling.removeClass("no_info");
					}
				}
			}
		}
